L.Control.StyledLayerControl = L.Control.Layers.extend({
    ...
    expandGroup: function(group_Name) { // PDB
        for (group in this._groupList) {
            if (this._groupList[group].groupName == group_Name) {
                for (layer in this._layers) {
                    if (this._layers[layer].group && this._layers[layer].group.name == group_Name) {
                        this._layers[layer].group.expanded = true
                    } else {
                        this._layers[layer].group.expanded = false
                    }
                }
                break;
            }
    ...
    _initLayout: function() {
        ...
        for (var c = 0; c < (containers = container.getElementsByClassName('ac-container')).length; c++) {
            ...
            this._default_maxHeight = this.options.container_maxHeight ? this.options.container_maxHeight : (this._map._size.y - 70); // PDB
    ...
    _on_resize_window: function() {
        ...
        for (var c = 0; c < containers.length; c++) {
            ...
            containers[c].style.maxHeight = (window.innerHeight - 90) < this._removePxToInt(this._default_maxHeight) ? (window.innerHeight - 90) + "px" : this._removePxToInt(this._default_maxHeight) + "px"; // PDB
        }
    },
    ...
    _addItem: function(obj) {
        ...
        if (obj.overlay) {
            if (obj.group.exclusive == false) { // PDB
                ...
                input = this._createRadioElement('leaflet-overlay-layers-' + obj.group.id, checked); // PDB

                label.className = "menu-item-radio"; // PDB 
            }
        } else {
        ...
        var groupContainer = this._domGroups[obj.group.id]; // PDB: Assign random ID to layers so we can have multiple controls
        ...
        if (!groupContainer) {
            ...
            inputElement = '<input id="ac' + random_id + '" name="accordion-1" class="menu" ' + s_expanded + s_type_exclusive + '/>'; // PDB: changed obj.group.id to random_id
            inputLabel = '<label for="ac' + random_id + '">' + obj.group.name + '</label>'; // PDB: changed obj.group.id to random_id
    ...
    _collapse: function() {
        // if (this._container.className.includes(' leaflet-control-layers-expanded')) {
        //     watersheds = UpdateWatersheds(0, this._map)
        //     if (this._map != null) {
        //         this._map.eachLayer(function(layer) {
        //             if (layer.id == 10001) {
        //                 layer.clearLayers();
        //                 layer.addLayer(watersheds);
        //             };
        //         });
        //     };
        // }
